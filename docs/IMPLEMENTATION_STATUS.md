...
`docker compose run --rm --no-deps -v "$PWD:/app" -e FOOTBALL_OBJECT_STORE_BACKEND=filesystem backend pytest -q` completed with 202 passed, two environment-gated tests skipped, and one known non-failing Starlette `TestClient` deprecation warning.
