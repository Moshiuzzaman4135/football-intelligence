...
`docker compose run --rm --no-deps -v "$PWD:/app" -e FOOTBALL_OBJECT_STORE_BACKEND=filesystem -e FOOTBALL_DATABASE_URL= backend pytest -q` completed with 220 passed, two environment-gated tests skipped, and one known non-failing Starlette `TestClient` deprecation warning.

## NEXT EXACT ACTION

Run the real-broadcast benchmark with YOLO11n (or the four-class football weight) once the RTX 3050 driver is reachable, comparing raw/pitch-filtered detections, peak confirmed tracks, and final events against the color-detector numbers recorded this session. Then add the conservative extra-event layer (possession/pass/ball-out) and qualify CALF action spotting on the remote RTX 3080. Distributed Celery execution, automatic ROI discovery, and pitch calibration remain deferred.
