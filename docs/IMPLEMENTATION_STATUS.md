...
Real-broadcast OCR score-change recovery. Diagnosed and fixed the real OCR failure (the real video's score changed but no event was emitted) and verified the fix on the real failing video `data/uploads/19b56077-cf8b-47bf-81ea-979d5bbd888a.mp4`. Root cause was threefold: the configured `.env` scoreboard ROI (center-top) reads only broadcast ads on this clip; the parser required a clock AND both team tokens on every single read; and `observe_missing` reset the pending window on any one miss. The consensus is now tolerant (bounded rolling window, bounded consecutive misses, clock/team best-effort, teams learned once and carried, placeholder refinement) and the parser accepts a score read with a single readable team token or a missing clock. The tolerant consensus now parses 19 real readings, learns teams `('ARS','MC')`, tolerates the many missed reads, and emits exactly one `score_change_candidate` (`0-0 -> 2-0`, needs_review) at t=112 s on the real video. CALF action spotting is now qualified and integrated behind an `ActionSpotter` interface (weights load, forward runs, 17-class label mapping, decode + NMS -> `FootballEvent`), and a semantic default timeline hides kick spam. Next exact action: add the isolated SoccerNet/ResNet feature extractor so `CalfActionSpotter` runs on arbitrary real video, then run a broadcast CALF benchmark.
...
DeepSeek session: (1) recovered the clean worktree, (2) diagnosed the real OCR score-change failure on `19b56077-...mp4` (ROI misconfigured, parser too strict, consensus reset on any miss), (3) made the scoreboard parser/consensus tolerant and verified a real `score_change_candidate`, (4) qualified CALF (weights load + forward + 17-class decode) and integrated it behind an `ActionSpotter` interface, (5) added evidence fusion (goal + score change => strong goal) and dedup/NMS, and (6) made the default timeline semantic with a debug toggle.

## LAST SUCCESSFUL COMMAND

Full protected suite through the backend image: `pytest -q` => 288 passed, 5 skipped (node/MinIO/Docker-gated), 1 known non-failing Starlette warning. `ruff check .` => All checks passed. `git diff --check` clean. Demo smoke test `football-intelligence process data/uploads/synthetic-football-demo.mp4` completed end to end.

## NEXT EXACT ACTION

Add the isolated SoccerNet ResNet-TF2 + PCA feature extractor so `CalfActionSpotter.spot` can accept a raw video path (it already accepts a `(frames, 512)` feature array). Then run a real-broadcast CALF benchmark and wire `fuse_semantic_events` into the full-match runner's event finalization. Distributed Celery execution, automatic ROI discovery, pitch calibration, and the four-class YOLO on GPU remain deferred.
