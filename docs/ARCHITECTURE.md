# Architecture

## Chosen shape

The first demonstrable release is a modular monolith: FastAPI owns lifecycle and persistence, a background executor runs one pipeline per job, Streamlit calls the API, SQLite stores metadata, and the filesystem stores source/output media. An in-process typed event bus emits semantic lifecycle and processing events. These boundaries permit later Redis/Celery deployment without inventing services before workload evidence requires them.

```text
Upload/local/RTSP -> source probe/frame iterator
                  -> Detector protocol -> Tracker protocol
                  -> temporal observations -> candidate rules -> fusion
                  -> SQLite metadata + annotated-frame renderer
                  -> FFmpeg H.264 MP4 -> FastAPI -> Streamlit timeline
```

## Components

- `domain.py`: Pydantic schemas for detections, tracks, evidence, events, video metadata, and jobs.
- `timebase.py`: frame/time conversion with explicit FPS validation.
- `bus.py`: synchronous semantic event envelope/publisher with subscriber isolation.
- `storage.py`: SQLite repositories and atomic job transitions; media paths remain outside SQL blobs.
- `video.py`: source validation, OpenCV iteration, metadata probe, and output finalization.
- `detection/base.py` and `detection/ultralytics.py`: normalized detector contract and YOLO adapter.
- `tracking/base.py` and `tracking/iou.py`: normalized contract and lightweight deterministic fallback; Ultralytics ByteTrack is the preferred ML path when installed.
- `events.py`: a continuous-track temporal kick candidate, deduplication, and independent-source evidence fusion. Further event types are later milestones.
- `overlay.py`: labels, confidence, team, event banners, timestamps, and trajectory trails.
- `pipeline.py`: orchestration, progress, stop checks, per-frame fault budget, metrics, and artifact completion.
- `api.py`: REST lifecycle/media endpoints and background execution.
- `object_store.py`: streaming multipart protocol with in-memory, filesystem, and S3/MinIO adapters.
- `uploads.py`: upload quota, ownership, expiry, part, checksum, completion, and delayed job-creation rules.
- `fullmatch/media.py`: same-bucket streamed localization, FFprobe validation, and bounded H.264 proxy preparation.
- `fullmatch/manifest.py` and `runner.py`: atomic immutable-option manifest, overlap-aware chunk checkpoints, bounded detection/tracking/event/overlay execution, resume, final concatenation/audio mux, and artifact validation.
- `fullmatch/ocr.py` and `heatmap.py`: manual-ROI Tesseract TSV evidence/score consensus and fixed 32x18 screen-space density artifacts.
- `ui.py`: upload/start/progress/video/timeline/evidence showcase.

## Normalized contracts

`Detector.detect(frame, frame_index, timestamp_ms) -> list[Detection]` and `Tracker.update(detections, frame_index, timestamp_ms) -> list[TrackObservation]` isolate vendor details. `TrackObservation.track_id` means visual continuity only. The event schema preserves raw evidence objects and producer names after fusion.

## Internal events

The current pipeline publishes `job.created`, `job.started`, `video.opened`, `detection.completed`, `tracking.updated`, `event.candidate`, `overlay.video.completed`, `job.completed`, `job.failed`, and `job.stopped`. The namespace reserves `frame.ready`, `clip.ready`, `event.fused`, and `overlay.frame.ready` for later producers. Handlers must not make frame processing fail; subscriber errors are isolated and logged.

## Error and fallback behavior

Invalid/unreadable sources and premature decode EOF fail visibly. Isolated inference/tracking/overlay failures increment a configured error budget and write the unchanged source frame, preserving output timing. Finalized media is probed before completion. Stop requests are checked between frames. Device selection follows local CUDA, optional remote experiment, smaller model, CPU, optional disablement. The mandatory path never calls SSH at runtime.

## Deployment

Docker Compose provides reproducible non-root CPU backend/UI containers plus persistent health-checked PostgreSQL and pinned MinIO on loopback-only host ports. A one-shot Alembic service must migrate an empty or existing database to head before the backend starts. Full-match clients request metadata-only upload sessions and transfer 16 MiB parts directly to presigned MinIO URLs; FastAPI streams the completed object to validate its declared SHA-256 before creating a job. Upload intent, private S3 identifiers, validated parts, expiry, and completion/job identity are durable SQLAlchemy records; compare-and-set transitions make restart and concurrent-worker recovery safe. A recoverable `finalizing` state prevents expiry cleanup from deleting media after its deterministic job insert begins. The internal MinIO service endpoint and host-visible presign endpoint are separate settings. Compose requires distinct runtime root and bucket-scoped application credentials and provisions only the application principal for the media bucket. The legacy short-clip filesystem upload remains available during migration. Native CUDA may be used when a compatible Python/PyTorch stack is verified. Redis/Celery, authentication for nonlocal deployment, and a remote inference API remain later production tasks.

FastAPI runs bounded expired-upload cleanup on its lifecycle timer. Operators should also configure a MinIO lifecycle rule that aborts incomplete multipart uploads after a period longer than the 24-hour application lease; it is a safety net for object-store fragments, not a rule that expires completed source videos.

The MVP full-match runner is an intentionally single-process executor with a separate one-job admission slot. It streams only an exact configured-bucket `s3://` key to an atomic local file, produces a 720p/25-or-lower proxy, and checkpoints every 120-second chunk with five seconds of context. Completed output intervals never overlap. Track IDs are namespaced per chunk; per-frame tracks are discarded after each frame and never written to SQL. The final H.264 video is re-timed from the non-overlap chunks, source/proxy audio is muxed once, and FFprobe must validate geometry, FPS, duration, codec, and audio before job completion. A committed manifest lets a newly created runner resume an orphaned `running` job without reprocessing completed chunks.
